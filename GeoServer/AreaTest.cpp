...
TEST_F(AnArea, AnswersUpperleftForBoundingRectangle)
{
    ...
    ASSERT_NEAR(Zero.go(HalfOneKm, West).longitude(), upperLeft.longitude(), Tolerance);
    ASSERT_NEAR(Zero.go(HalfTwoKm, North).latitude(), upperLeft.latitude(), Tolerance);
}
...
TEST_F(AnArea, AnswersUpperRightForBoundingRectangle)
{
    ...
    ASSERT_NEAR(Zero.go(HalfOneKm, East).longitude(), upperRight.longitude(), Tolerance);
    ASSERT_NEAR(Zero.go(HalfTwoKm, North).latitude(), upperRight.latitude(), Tolerance);
}
...
TEST_F(AnArea, AnswersLowerRightForBoundingRectangle)
{
    ...
    ASSERT_NEAR(Zero.go(HalfOneKm, East).longitude(), lowerRight.longitude(), Tolerance);
    ASSERT_NEAR(Zero.go(HalfTwoKm, South).latitude(), lowerRight.latitude(), Tolerance);
}
...
TEST_F(AnArea, AnswersLowerLeftForBoundingRectangle)
{
    ...
    ASSERT_NEAR(Zero.go(HalfOneKm, West).longitude(), lowerLeft.longitude(), Tolerance);
    ASSERT_NEAR(Zero.go(HalfTwoKm, South).latitude(), lowerLeft.latitude(), Tolerance);
}

TEST_F(AnArea, DesignatesAPointOutOfBoundsWhenTooFarNorth)
{
    Location tooNorth{Zero.go(HalfTwoKm + 10, North).latitude(), 0};
    ASSERT_FALSE(OneByTwoAreaAtZero.inBounds(tooNorth));
}

TEST_F(AnArea, DesignatesAPointInBoundsWhenAtNorth)
{
    Location northEdge{Zero.go(HalfTwoKm, North).latitude(), 0};
    ASSERT_TRUE(OneByTwoAreaAtZero.inBounds(northEdge));
}

TEST_F(AnArea, DesignatesAPointoutOfBoundsWhenTooFarSouth)
{
    Location tooSouth{Zero.go(HalfTwoKm + 10, South).latitude(), 0};
    ASSERT_FALSE(OneByTwoAreaAtZero.inBounds(tooSouth));
}
